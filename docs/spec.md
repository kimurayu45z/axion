...
```
"hello"              // str（静的文字列スライス）
`hello {name}`       // String（テンプレートリテラル、{} 内の式を埋め込む）
```

- `""` は `str` 型を返す（静的データ、補間なし）
- バッククォート `` ` `` はテンプレートリテラル。`String` 型を返す（ヒープ割り当て）
- テンプレートリテラル内で `{式}` を使って任意の式を埋め込める（TypeScript に類似）
...
バッククォート文字列内で `{式}` を使って任意の式を埋め込める。

```
let name = "world"
let msg = `hello {name}`              // → "hello world"
let calc = `result: {1 + 2}`         // → "result: 3"
let method = `len: {name.len()}`     // → "len: 5"
let nested = `a: {foo(x, y)}`        // 関数呼び出しも可
```
...
`{` をリテラルとしてテンプレートリテラルに含めたい場合は `\{` でエスケープする：
